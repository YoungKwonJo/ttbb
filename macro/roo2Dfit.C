void roo2Dfit(int iVari=0,TString weight="csvweight",double R_nomal=0.0229614, bool separatettb=true){ // bb
//void roo2Dfit(int iVari=0,TString weight="csvweight",double R_nomal=0.0213862, bool separatettb=true){ //tth
  gSystem->Load("libRooFit") ;
  using namespace RooFit ;

  TFile* f = new TFile(Form("output_%s.root",weight.Data()));
//  TFile* f = new TFile("output.root");
 ...
 // bool separatettb = true;
  ...
  if(iVari==3) { xy_ttcc->Scale(2); }
  if(iVari==4) { xy_ttcc->Scale(0); }
...
*/
  ...
  RFrame->SetMaximum(5);RFrame->SetMinimum(0);
  RFrame->Draw();
...
//  nllk->plotOn(kFrame);
  ...
  kFrame->SetMaximum(5);kFrame->SetMinimum(0);
  kFrame->Draw();
